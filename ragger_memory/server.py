...
def _get_memory(username: str = None):
    ...
    if username not in _user_memories:
        ...
        # Try to acquire housekeeping lock for this user (if server is running)
        if hasattr(_get_memory, '_hk_acquire'):
            _get_memory._hk_acquire(username)
...
def run_server(host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
    ...
    def _acquire_user_hk_lock(username):
        ...
        try:
            ...
            _hk_locks[username] = fd
            print(f"Housekeeping owner for user '{username}'")
            return True
    ...
    def _housekeeping_loop():
        while True:
            ...
            try:
                ...
                user_db_paths = set()
                for username, um in _user_memories.items():
                    if username in _hk_locks and um and um._user_backend and hasattr(um._user_backend, 'db_path'):
                        user_db_paths.add(um._user_backend.db_path)
                results = run_housekeeping(
                    memory=_memory,
                    inference_client=_inference_client,
    ...
    try:
        ...
        try:
            ...
            pass
        for username, fd in _hk_locks.items():
            try:
